/* ═══════════════════════════════════════════════
   ...
   Cakra v1.1 — with 5G NR layer + theme support
═══════════════════════════════════════════════ */
...
window.GNetMap = (() => {
  ...
  // Cached thinned dataset + per-point analysis flags
  // gap/churn: Set of indices into _thinned (binary on/off)
  // throughputTier: Map<index, 'low'|'mid'|'high'> — 3-tier per docs
  let _thinned = [];
  ...
  // Update tile layer ketika theme berubah tanpa rebuild map
  ...
  function build(DATA) {
    ...
    // Dedicated pane for analysis rings — sits ABOVE the default overlay pane
    // (z=400) so rings stay visible regardless of which signal layer is active
    if (!map.getPane('analysisRings')) {
      map.createPane('analysisRings');
      map.getPane('analysisRings').style.zIndex = 460;
      map.getPane('analysisRings').style.pointerEvents = 'none'; // click pass-through to signal pts
    }
    ...
    const thin = GNetParser.thin(gps, 1200);
    _thinned = thin;
    thin.forEach(d => {
    ...
    // Compute analysis flags on the thinned set, then build ring overlays
    // (NOT added to map by default — toolbar toggle controls visibility)
    ...
    L.marker(e.latlng, {
      ...
      <div style="font-family:var(--mono);font-size:11px;color:#06b6d4;max-width:180px;word-wrap:break-word">
        ...
        <div style="color:var(--popup-label,#50505a);font-size:10px;margin-top:6px">
          ...
          <button onclick="GNetMap.deleteAnnotation(this)" style="margin-top:6px;padding:4px 8px;background:var(--red-dim);color:#f87171;border:none;border-radius:3px;cursor:pointer;font-family:var(--mono);font-size:9px">Hapus</button>
        </div>
  ...
  // ══════════════════════════════════════════
  // ANALYSIS OVERLAY RINGS  (Opsi A)
  // Pre-compute per-point flags on the thinned set, then render rings
  // in a dedicated pane that sits above any active signal layer.
  // Toggleable & non-exclusive — gap/churn/throughput can stack.
  // ══════════════════════════════════════════
  // Throughput tier visual style — sesuai docs:
  // "marker besar hijau = throughput tinggi, marker kecil merah = throughput rendah"
  // Ring lebih luar = high (good, subtle); ring lebih ketat = low (concerning, prominent)
  const TP_TIER_STYLE = {
  ...
  function computeAnalysisFlags(opts) {
    ...
    // — Coverage gap: consecutive run of points with RSRP <= threshold (length >= 2)
    let runStart = -1;
    for (let i = 0; i < n; i++) {
    ...
    // — Cell churn / handover instability:
    //   >=3 unique serving cells dalam trailing window (windowSize+1 sample).
    //   NB: ini BUKAN klasik pilot pollution (yang butuh data neighbor cell
    //   measurement). Yang dideteksi adalah area instabilitas serving cell
    //   yang bisa disebabkan pilot pollution, mobility, atau misconfig HO.
    ...
    // — Throughput tier: 3-tier per docs (high >66%, mid 33-66%, low <33% of peak DL)
    let maxDl = 0;
  ...
  // Map toolbar key → internal layer key
  ...
  function updateOverlayBadges() {
    // Gap & Cell Churn — simple count badges
  ...
  // ══════════════════════════════════════════
  // COVERAGE GAP DETECTION (legacy standalone polyline layer)
  // Kept for backwards-compat; not wired to the new toolbar overlay system.
  // ══════════════════════════════════════════
  ...
  // ══════════════════════════════════════════
  // CELL CHURN / HANDOVER INSTABILITY DETECTION
  // (legacy standalone marker layer — kept for backwards compat with
  //  switchAnalysisLayer; new ring overlay system is in buildRingOverlays)
  //
  // Detects points where serving cell rotates rapidly (>= N unique cells in a
  // sliding window). NOTE: ini BUKAN klasik pilot pollution per definisi
  // industri (yang butuh data measurement neighbor cell scanner). Lihat
  // dokumentasi untuk perbedaannya.
  // ══════════════════════════════════════════
  function buildCellChurnLayer(DATA, windowSize) {
    ...
    const win = windowSize || 10; // look-back window in data points
    let churnCount = 0;
  ...
  // ══════════════════════════════════════════
  // THROUGHPUT vs RSRP SCATTER
  // Shows DL throughput at each GPS point
  // sized & colored by throughput value
  // ══════════════════════════════════════════
  ...
  function buildAnalysisLayers(DATA, opts) {
    ...
    // Recompute the per-point flag set + rebuild rings so threshold changes
    // from analysis panels propagate to the overlay system instantly
    ...
    buildRingOverlays();
    // Refresh button states (counts may have changed)
